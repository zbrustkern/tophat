...
export default function Dashboard() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<{ type: PlanType, id: string | null }>({ type: 'income', id: null });
  ...
  const renderPlanComponent = () => {
    switch (selectedPlan.type) {
      case 'income':
        return <ClientIncomePlanPage planId={selectedPlan.id || undefined} />;
      case 'savings':
        return <ClientSavingsPlanPage planId={selectedPlan.id || undefined} />;
      default:
        return null;
    }
  };
    ...
    <main className="p-4">
      ...
      <div className="mb-4 flex space-x-2">
        {PLAN_TYPES.map((planType) => (
          <Button key={planType} onClick={() => setSelectedPlan({ type: planType, id: null })}>
            Create New {PLAN_TYPE_LABELS[planType]} Plan
          </Button>
        ))}
      </div>
      {PLAN_TYPES.map((planType) => (
        <div key={planType}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {plans
              ...
              .map((plan) => (
                <Card key={plan.id} onClick={() => setSelectedPlan({ type: planType, id: plan.id })}>
                  <CardHeader>
